...
def generate_midday_ai(movers: list[dict], news: list[dict]) -> str:
    """
    Nhận định giữa phiên — dùng chung personas với cuối phiên/trước phiên.
    movers: list[{ticker, pct_chg, close, turnover}]
    news:   list[{title, source}]
    """
    try:
        client = _get_client()
    except Exception as e:
    ...
    personas = {
        0: (
            "Bạn là dân văn phòng vừa ăn trưa xong, đang lướt app chứng khoán trước giờ họp chiều.",
            "Giọng mệt mỏi nhưng tỉnh, hay so sánh với deadline và sếp. Bắt đầu ngay bằng 1 mã cụ thể.",
            "Câu cuối: nói thẳng phiên chiều nên làm gì, giọng đồng nghiệp nói nhau.",
        ),
        1: (
            "Bạn là bà nội trợ vừa đi chợ về, tranh thủ xem chứng khoán trước khi nấu cơm trưa.",
            "So sánh với chợ búa: giá rau, mặc cả, hàng ế. Bắt đầu ngay vào số liệu cụ thể.",
            "Câu cuối: lời khuyên kiểu bà buôn bán lâu năm, thực tế.",
        ),
        2: (
            "Bạn là tiểu thương vừa đóng cửa hàng nghỉ trưa, đang tổng kết buổi sáng.",
            "So sánh với buôn bán: lời lỗ, hàng tồn, vốn xoay vòng. Nói thẳng, không vòng vo.",
            "Câu cuối: nên hold hay cắt, nói kiểu tính toán vốn xoay vòng.",
        ),
        3: (
            "Bạn là Gen Z vừa ăn trưa xong, đang nhắn nhóm ae về diễn biến sáng nay.",
            "Dùng ngôn ngữ Gen Z: toang, xanh đỏ loạn, FOMO. Thẳng, hài, không cần lịch sự.",
            "Câu cuối: nhắn ae nên làm gì phiên chiều, giọng group chat.",
        ),
        4: (
            "Bạn là người nông dân nghỉ trưa dưới bóng cây, nhìn chứng khoán như nhìn mùa màng.",
            "So sánh với lúa, thời tiết, mùa vụ. Mộc mạc, thật thà, bắt đầu ngay vào mã cụ thể.",
            "Câu cuối: nên giữ hay bán, nói kiểu ông nông dân tính thu hoạch.",
        ),
    }
    setup, style, ending = personas.get(weekday, personas[0])
    ...
    for m in movers[:15]:
        pct = float(m["pct_chg"])
        arrow = "▲" if pct > 0 else "▼"
        tk = float(m.get("turnover") or 0) / 1e9
        mover_lines.append(
            f"- {m['ticker']}: {arrow}{abs(pct):.1f}% | giá {fmt_price(float(m['close']))} | TK {tk:.1f} tỷ"
        )
    news_lines = [f"- [{n['source']}] {n['title']}" for n in news[:8]]

    prompt = textwrap.dedent(f"""
        {setup}
        Phong cách: {style}

        SỐ LIỆU GIỮA PHIÊN {today}:
        Các mã biến động mạnh (≥2%) sáng nay, xếp theo thanh khoản:
        {chr(10).join(mover_lines)}

        Tin tức liên quan:
        {chr(10).join(news_lines) if news_lines else "Chưa tìm thấy tin tức nổi bật."}

        RÀNG BUỘC BẮT BUỘC:
        - Tiếng Việt có dấu đầy đủ, không emoji, không bullet, không markdown
        - TUYỆT ĐỐI không mở đầu bằng: "Hôm nay là một ngày", "Thị trường hôm nay"
        - TUYỆT ĐỐI không kết bằng: "tôi sẽ tiếp tục theo dõi", "cần phân tích kỹ lưỡng"
        - Đề cập ít nhất 2 mã cụ thể kèm số %
        - Tối đa 4 câu văn xuôi liền mạch
        - {ending}
    """).strip()

    logger.info("AI: nhận định giữa phiên...")
    return _call(client, prompt, max_tokens=400, _temperature=0.55)
